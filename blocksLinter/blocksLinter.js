...
const nodeRecognizer = (blockName, modName, modValue, ...otherProperties) => {
  if (!arguments.length) {
    return () => true;
  }
  ...
  if (modName) {
    const modRecognizer = modValue ? node => node.mods && node.mods[modName] === modValue : node => node.mods && node.mods[modName] ? true : false;
  ...
  if (otherProperties.length) {
    otherProperties.forEach(property => {
      ...
      const propertyRecognizer = propertyValue ? node => node[propertyKey] === propertyValue : node => node[propertyKey] ? true : false;
...
function findNodeIn(rootNode, nodeRecognizer) {
  if (!nodeRecognizer) {
    return;
  }
...
function findAllElementsInBlock(block, elemName) {
  const elementRecognizer = nodeRecognizer(null, null, null, {
    elem: elemName
  });
...
function checkTextDifference(warningBlock) {
  ...
  return LinterError.getErrorsForBlocksWithOneLocation(warning.textSize, invalidBlocks, warningBlock);
}
...
function checkButtonSize(warningBlock) {
  ...
  return LinterError.getErrorsForBlocks(warning.buttonSize, invalidButtons);
}
...
function checkButtonPosition(warningBlock) {
  ...
  return LinterError.getErrorsForBlocks(warning.buttonPosition, invalidButtons);
}
...
function checkPlaceholderSize(warningBlock) {
  ...
  return LinterError.getErrorsForBlocks(warning.placeholderSize, invalidPlaceholders);
}

const checkers = {
  textDifference: checkTextDifference,
  buttonSize: checkButtonSize,
  buttonPosition: checkButtonPosition,
  placeholderSize: checkPlaceholderSize
};
...
function checkH1Severalty(rootNode) {
  ...
  return LinterError.getErrorsForBlocks(text.severalH1, h1Headers.slice(1));
}

const h1HeaderRecognizer = nodeRecognizer('text', 'type', 'h1');
const h2HeaderRecognizer = nodeRecognizer('text', 'type', 'h2');
const h3HeaderRecognizer = nodeRecognizer('text', 'type', 'h3');
...
function checkH2Position(rootNode) {
  ...
  return LinterError.getErrorsForBlocks(text.h2Position, invalidH2Headers);
}
...
function checkH3Position(rootNode) {
  ...
  return LinterError.getErrorsForBlocks(text.h3Position, invalidH3Headers);
}

const checkers$1 = {
  h1Severalty: checkH1Severalty,
  h2Position: checkH2Position,
  h3Position: checkH3Position
};
...
function checkAds(gridBlock) {
  ...
  return new LinterError(grid.advertisements, gridBlock);
}

const checkers$2 = {
  advertisements: checkAds
};
...
class Linter {
  ...
  text(rootNode) {
    ...
    const errors = [...h1Severalty(rootNode), ...h2Position(rootNode), ...h3Position(rootNode)];
    return errors;
  }
...
function getASTRoots(json) {
  let astRootObject = {};

  try {
    astRootObject = build(json);
  } catch (error) {
...
function getRootNodesFromJSON(json) {
